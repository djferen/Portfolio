...
int main() {
  ...
  std::cout << "Searched for a" << (result ? " found" : " not found")
            << std::endl;

  result = myBinaryTree.search("b");
  std::cout << "Searched for b" << (result ? " found" : " not found")
            << std::endl;

  result = myBinaryTree.search("f");
  std::cout << "Searched for f" << (result ? " found" : " not found")
            << std::endl;

  result = myBinaryTree.search("h");
  std::cout << "Searched for h" << (result ? " found" : " not found")
            << std::endl;

  myBinaryTree.print();

  binarytree<std::string>::binarytree_link link = myBinaryTree.searchlink("a");
  std::cout << "Searched for a" << (link != nullptr ? " found" : " not found")
            << std::endl;

  link = myBinaryTree.searchlink("zz");
  std::cout << "Searched for zz" << (link != nullptr ? " found" : " not found")
            << std::endl;
